package com.axway.apigw.android.db;
...
public class DbProvider extends ContentProvider {
	...
	public static final String CONTENT_AUTHORITY = "com.axway.apigw.android";
	...
	private static UriMatcher buildUriMatcher() {
		...
		rv.addURI(auth, DbHelper.Tables.CONN_MGR, CONN_MGR);
		rv.addURI(auth, DbHelper.Tables.CONN_MGR + "/*", CONN_MGR_ID);
	...
	public int delete(Uri uri, String selection, String[] selectionArgs) {
		...
		switch (n) {
			case CONN_MGR:
				tblName = DbHelper.Tables.CONN_MGR;
			break;
			case CONN_MGR_ID:
				tblName = DbHelper.Tables.CONN_MGR;
			break;
		...
		if (id == 0) {
			...
			rv = db.delete(tblName, DbHelper.CommonColumns._ID + "=" + Long.toString(id) + (TextUtils.isEmpty(selection) ? "" : " AND (" + selection + ")"), selectionArgs);
	...
	public String getType(Uri uri) {
		...
		switch (n) {
			case CONN_MGR:
				rv = DbHelper.ConnMgrColumns.CONTENT_TYPE;
			break;
			case CONN_MGR_ID:
				rv = DbHelper.ConnMgrColumns.CONTENT_ITEM_TYPE;
			break;
		}
		return rv;
	}
	...
	private void cleanCommonValues(ContentValues values) {
		...
		if (!values.containsKey(DbHelper.CommonColumns.MODIFY_DATE))
			values.put(DbHelper.CommonColumns.MODIFY_DATE, now);
	...
	public Uri insert(Uri uri, ContentValues valuesIn) {
		...
		switch (n) {
			case CONN_MGR:
				...
				tblName = DbHelper.Tables.CONN_MGR;
				baseUri = DbHelper.ConnMgrColumns.CONTENT_URI;
			break;
			case CONN_MGR_ID:
				cleanConnMgrValues(values);
				tblName = DbHelper.Tables.CONN_MGR;
		...
		if (tblName == null || baseUri == null)
			return rv;
		if (values.containsKey(DbHelper.CommonColumns._ID))
	...
	public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
		...
		int n = uriMatcher.match(uri);
		if (projection == null)
			projection = DbHelper.ConnMgrColumns.LIST_PROJECTION;
		switch (n) {
			case CONN_MGR:
				tblName = DbHelper.Tables.CONN_MGR;
				if (sortOrder == null)
					sortOrder = DbHelper.ConnMgrColumns.HOST + " ASC, " + DbHelper.ConnMgrColumns.PORT + " DESC";
			break;
			case CONN_MGR_ID:
				tblName = DbHelper.Tables.CONN_MGR;
				if (selection == null) {
					selection = DbHelper.ConnMgrColumns._ID + " = ?";
					selectionArgs = new String[] { Long.toString(id) };
				}
				else {
					selection = selection + " AND (" + DbHelper.ConnMgrColumns._ID + " = " + Long.toString(id) + ")";
				}
			break;
		}
		if (tblName == null)
